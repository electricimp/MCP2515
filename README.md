...
This library provides a driver for the [Microchip MCP2515](https://www.microchip.com/wwwproducts/en/en010406), a stand-alone CAN controller with a SPI interface. The MCP2515 is capable of transmitting and receiving both standard and extended data and remote frames. It has three transmit buffers with prioritization and abort features and two receive buffers. To filter out unwanted messages, the MCP2515 has two acceptance masks and six acceptance filters. You can [download the MCP2515 Datasheet here](http://ww1.microchip.com/downloads/en/DeviceDoc/MCP2515-Stand-Alone-CAN-Controller-with-SPI-20001801J.pdf).

**Note** This driver is still under development, so not all of the MCP2515’s features have yet been implemented. Currently, message receive and very basic transmit are implemented, and only 10MHz clock [timing suggestions](#timing-suggestions) have been tested.

**To include this driver in your project, copy and paste the contents of the** `MCP2515.device.lib.nut` **file at the top of your device code.**

## Class Usage ##

### constructor(*spiBus[, chipSelectPin]*) ###

The driver is instantiated  with a configured imp SPI bus object and, optionally, a imp pin object to which the Chip Select (CS) line is connected. The chip select pin will be configured by the constructor. If no CS pin is provided, the imp API method [**spi.chipselect()**](https://developer.electricimp.com/api/hardware.spi/chipselect) will be used to drive the CS pin.

**Note** This method is only available on imps with a dedicated chip select pin and can only be used when the SPI bus is configured with **USE_CS_L** mode flag.
...
This method initializes the MCP2515 based on the settings specified, if any, in the table passed into *settings*. It will reset the MCP2515 to get into an known state before applying the settings. If no settings table is passed in it will default to settings for a 10MHz clock with a transmit speed of 1000Kb/s, with no receive message filtering or interrupts enabled. For settings for other clock and speeds, please see [‘Timing Notes’](#timing-notes), below.
...
| --- | --- | --- |
| *enFiltering* | Boolean  | Whether to enable filtering (`true`) or not (`false`). Default: `false` |
| *opMode* | Constant | Sets the operation mode after configuration. For supported values, please see [‘Operation Mode Constants’](#operation-mode-constants), below. Default: *MCP2515_OP_MODE_NORMAL* |
...
| *samConfig* | Constant | Sample point Configuration. Determines whether the bus line is sampled one or three times at the sample point. Supported values: *MCP2515_SAM_1X* or *MCP2515_SAM_3X*. Default: *MCP2515_SAM_3X* |
...
A mode constant &mdash; This is the mode the chip is set to after updating the mode. Use this to confirm the mode is set correctly.
...
| Parameter | Type | Required? | Description |
| --- | --- | --- | --- |
| *settings* | Constant(s) | Yes | Use the [interrupt configuration constants](#interrupt-configuration-constants) below to select the desired interrupt(s). If more than one interrupt is required, the constants can be OR’d (see example, below) |
...
| *MCP2515_EN_INT_ERR* | Enables an interrupt when other errors occur |
...
| Parameter | Type | Required? | Description |
| --- | --- | --- | --- |
| *settings* | Constant(s) | Yes | Use any of the [RX buffer pin constants](#rx-buffer-pin-constants) below to select the desired pin configuration. With the exception of *MCP2515_RXBF_PINS_DISABLE*, constants can be OR’d to include multiple values (see example, below) |
...
| Parameter | Type | Required? | Description |
| --- | --- | --- | --- |
| *settings* | Constant(s) | Yes | Use any of the [TX RTS pin constants](#tx-rts-pin-constants) below to select the desired pin configuration settings. RTS pin constants can be OR’d to include multiple values |
...
### configureMask(*maskNum, maskID*) ###

This method configures the masks which are used to filter incoming messages.

**Note** The [*init()*](#initsettings) method configures all mask registers to `0`, bypassing filters. As such, all messages will be accepted until masks have been configured.

#### Parameters ####

| Parameter | Type | Required? | Description |
| --- | --- | --- | --- |
| *maskNum* | Integer | Yes | Accepted values: 0 or 1. Mask 0 will impact messages loaded into RX Buffer 0. Mask 1 will impact messages loaded into RX Buffer 1 |
| *maskID* | Integer bitfield | Yes | For a given bit, if it is set to `1`, messages will only be loaded into the RX buffer if they match the filters for that buffer. If the bit is cleared, filters for that buffer will be bypassed |
...
### configureFilter(*filterNum, extended, filterID*) ###

This method configures filters that will be applied to incoming messages.

**Note** The [*init()*](#initsettings) method configures all mask registers to `0`, bypassing filters.  For filters to take effect, masks must be configured.

#### Parameters ####

| Parameter | Type | Required? | Description |
| --- | --- | --- | --- |
| *filterNum* | Integer | Yes | Accepted values: 0-5. Filters 0-1 will impact messages loaded into RX Buffer 0. Filters 2-5 operate upon messages loaded into RX Buffer 1 |
| *extended* | boolean | Yes | If `true`, the filter will be applied to incoming extended messages; if `false`, the filter will be applied to incoming standard messages |
| *filterID* | Integer | Yes | Incoming messages with IDs that match the *filterID* will be loaded into the receive buffer |
...
This method checks for messages. If a message is found, it is returned, otherwise the method returns `null`.
...
| Parameter | Type | Required? | Description |
| --- | --- | --- | --- |
| *message* | Table | Yes | See [below](#message-table) for the values you must include in the message table |
...
2. PropSeg + PS1 ≥ PS2 &mdash; This requirement ensures the sample point is greater than 50% of the bit time.
...
| Clock&nbsp;Rate | Bus&nbsp;Speed | Baud&nbsp;Rate&nbsp;Pre-scaler | PropSeg | Phase&nbsp;Segment 1 | Phase&nbsp;Segment 2 | SJW |
| ---------- | --------- | ------------------- | ------------------- | --------------- | --------------- | ------------------------- |
